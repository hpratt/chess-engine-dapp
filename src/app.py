    ...
    with chess.engine.SimpleEngine.popen_uci("/usr/local/bin/stockfish") as engine:
        if cargs.uci_moves is not None:
            ...
            for move in cargs.san_moves:
                board.push_san(move)
        analysis = engine.analyse(board, chess.engine.Limit(time = cargs.time_limit, depth = cargs.depth), multipv = cargs.pv_count)

    ### format results and write to the specified output location
    result = json.dumps([{
        "depth": x["depth"],
